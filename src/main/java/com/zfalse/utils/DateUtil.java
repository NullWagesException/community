...
public class DateUtil {
    ...
    /**
     * 当前周的开始时间 yyyy:MM:dd 00:00:00
     * @return
     */
    public static List<Date> getWeekStart(){
        List<Date> list = new ArrayList<>();
        Calendar calendar = new GregorianCalendar();
        int day = calendar.get(Calendar.DAY_OF_WEEK);
        list.add(getNowDayStart());
        for(int i = -1;i>-day;i--){
    ...
    public static void main(String[] args) {
        List<Date> weekStart = getWeekStart();
        for (Date date : weekStart) {
            System.out.println(date);
        }
    }

    /**
     * 当前周的结束时间 yyyy:MM:dd 23:59:59
     * @return
     */
    ...
    public static Integer dateToInteger(Date date) {
        String result = "";
        SimpleDateFormat formater = new SimpleDateFormat(FORMAT_FOUR);
        try {
            result = formater.format(date);
        } catch (Exception e) {
            // log.error(e);
